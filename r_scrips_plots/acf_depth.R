...
df_well <- df[df$Well_ID == "GMW000000010097", ]
df_well <- df_well[order(as.Date(df_well$Date)), ]

# Compute ACF
n <- nrow(df_well)
acf_obj <- Acf(df_well$Depth, lag.max = 80, plot = FALSE)
...
# Build dataframe (subsample every 3rd lag)
df_acf <- data.frame(
  lag = lags,
  acf = acf_vals,
  ci = ci_bounds
